...
| 你想做什么 | 方式一：提 Issue（推荐，不用 Fork） | 方式二：提 PR |
|---|---|---|
...
两条路殊途同归：机器人自动校验（规则完全一样），通过就自动生效，全程无需人工审核。

- **Issue 最省事**：打开 [apply.html](./apply.html) 表单，选好操作模式，标题正文全部自动生成，登录 GitHub 点提交就行
- **PR 适合熟悉 Git 的人**：本地改文件、走常规 PR 流程，详见下方「用 PR 操作」一节
...
两种办法任选（不想用 Issue 也可以走 PR，见后文「用 PR 操作」）：
...
比如换头像、改名、改简介。两种办法任选（PR 方式：直接改你自己的文件提 PR，见「用 PR 操作」）：
...
适合熟悉 Git 的用户，走常规 PR 流程：Fork 仓库 → 在 `data/friends/` 下改文件 → 提 PR → 校验通过自动合并。

### 添加友链

第 1 步和 Issue 方式相同：先把本站链接挂到你的友链页（PR 校验同样会抓取检查，没有就拒绝）。
...
删掉你自己的那个 JSON 文件，提交 PR。

### PR 的校验规则

和 Issue 用同一套校验（回链、可达性、域名所有权验证），另有几条 PR 专属限制：

- 一个 PR 只能改**一个文件**，且必须在 `data/friends/` 目录下
- 不能带 `vip` 字段——那是站主专用，带了会被直接拒绝
- 修改/删除需要域名所有权验证，验证码绑定 PR 编号（`moara-friends=<PR编号>`）
- 校验失败会在 PR 里评论原因；改完 push 上去自动重新校验，关闭的 PR 重新打开也能触发
- 之前 Fork 过的话，先 Sync fork 同步最新代码再改，避免冲突
