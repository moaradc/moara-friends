...
沫然Blog 的友链仓库。提交 Issue 或 PR 即可申请友链，自动校验
...
|---|---|---|
| 申请友链 | 标题 `[Friend Link] 站点名`，正文填完整资料 | 新建 `data/friends/你的文件.json` |
...
- **手动提**：新建 Issue，标题以 `[Edit]` 开头，正文格式和申请友链一样，但 Filename 必须填**已收录的那个文件名**（去 `data/friends/` 目录找你的文件）
